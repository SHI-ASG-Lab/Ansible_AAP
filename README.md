...
2.  Download the repo to the cloud shell with git or upload the AAP.tf file through the shell.  The only files needed in the cloud shell is the AAP.tf.  Ignore the prep scripts or delete them from the cloud shell.  You can create a terraform.tfvars file with the variables and values for faster deployment.  Variables are written - <variable name> = "value"
